<script lang="ts">
  ...
  $: props = {
    class: spreadClassnames([
      "gbs-menu-item",
      "gbs-body-primary",
      $$slots.avatar && "gbs-menu-item--avatar",
      disabled && "disabled",
      className,
    ]),
  };
...
<li {...props}>
  ...
  >
    ...
    {#if $$slots.label}
      <div class="gbs-body-tertiary menu-item-label-container" class:disabled>
        <slot name="label" />
      </div>
    {:else if $$slots.helper}
      <div class="gbs-body-tertiary menu-item-helper-container">
        <Tooltip variant="descriptive" position={helperTooltipPositioning}>
          <Icon
            color={disabled
...
<style>
  ...
  .gbs-menu-item--avatar .gbs-menu-item-button {
    align-items: center;
  }

  .gbs-menu-item:hover:not(.disabled) .gbs-menu-item-button {
