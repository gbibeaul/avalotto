<script type="ts">
    ...
    $: if (fullWidth) {
        ...
        underlineWidth = `${
            $currentTheme !== 'fv-classic' ? $longestTab : 0
        }px`;
    }
    ...
    $: props = {
        ...
        class: spreadClassnames([
            'fvs-tab-list',
            fullWidth && 'fvs-tab-list--full-width',
            $$restProps.class,
        ]),
        style: cssVarStyles,
    };
...
<style>
    ...
    .fvs-tab-list :global(button:not(:last-of-type)) {
    ...
    .fvs-tab-list--full-width :global(.fvs-tab) {
        max-width: none;
    }
</style>
