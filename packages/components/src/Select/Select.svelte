<script lang="ts">
  ...
  $: cssVarStyles = `
        --gbs-menu-text-align:${textAlignment};
        --gbs-menu-width:${selectWidth}px;
    `;
  ...
  $: props = {
    ...
    class: spreadClassnames([
      "gbs-select",
      invalid && "gbs-select--error",
      disabled && "gbs-select--disabled",
      textAlignment === "right" && "gbs-select--text-align-right",
      $$restProps.class,
    ]),
    style: cssVarStyles,
  };
...
<div {...props} use:clickOutside={dismissMenu}>
  {#if label}
    <label for={id} class="select-label gbs-heading-75">{label}</label>
  {/if}
  ...
  {#if menuOpen}
    <MenuSurface {position} animation={slide} class="gbs-select-surface">
  ...
  {#if helperText}
    <small class="helper-field">
      <span class="gbs-text-field-helper-icon">
        <Icon size="tertiary" class="gbs-select-helper-icon">help_outline</Icon>
      </span>
      {helperText}
    </small>
  {/if}
...
<style>
  .gbs-select {
    ...
    width: var(--gbs-menu-width);
  }

  .gbs-select :global(.select-dropdown-arrow-icon) {
  ...
  .gbs-select--text-align-right .select-button {
  ...
  .gbs-select--error .select-button {
    border-color: var(--t-color-border-select-error);
  }

  .gbs-select--disabled .select-button {
  ...
  .gbs-select :global(.gbs-select-option) {
    text-align: var(--gbs-menu-text-align);
  }
  ...
  .gbs-select--error :global(.select-icon) {
    color: var(--t-color-border-select-error);
  }

  .gbs-select--disabled :global(.select-icon) {
    color: var(--t-color-text-disabled);
  }
  ...
  .gbs-select--error .helper-field,
  .gbs-select--error .select-label {
    color: var(--t-color-border-select-error);
  }

  .gbs-select--disabled .helper-field,
  .gbs-select--disabled .select-label {
    color: var(--t-color-text-select-disabled);
  }

  .gbs-select--disabled :global(.select-dropdown-arrow-icon) {
    color: var(--t-color-text-select-disabled);
  }

  .gbs-select :global(.gbs-select-helper-icon) {
    color: var(--t-color-helper-select-default);
  }

  .gbs-select--error :global(.gbs-select-helper-icon) {
    color: var(--t-color-border-select-error);
  }

  .gbs-select--disabled :global(.gbs-select-helper-icon) {
    color: var(--t-color-helper-select-disabled);
  }

  .gbs-select :global(.gbs-select-surface .scrollable-content) {
    overflow-y: overlay;
  }
</style>
