<script type="ts">
  ...
  $: if (menuSurface) {
    if (menuSurfaceRight > windowWidth - MINIMUM_DISTANCE_FROM_VIEWPORT) {
      offsetHorizontal =
        windowWidth - (menuSurfaceRight + MINIMUM_DISTANCE_FROM_VIEWPORT);
  ...
  $: props = {
    ...
    class: spreadClassnames([
      "gbs-menu-surface",
      position && `gbs-menu-surface--${position}`,
      $$restProps.class,
    ]),
    style: cssVarStyles,
  };
...
<style>
  .gbs-menu-surface--right {
    transform: translate(var(--menu-offset));
  }

  .gbs-menu-surface--right .scrollable-content {
    transform-origin: right top;
  }

  .gbs-menu-surface--left {
    left: var(--menu-offset);
  }

  .gbs-menu-surface--left .scrollable-content {
  ...
  .scrollable-content {
    ...
    box-shadow: 0px 4px 5px rgba(0, 0, 0, 0.14),
      0px 2px 4px -1px rgba(0, 0, 0, 0.12), 0px 1px 10px rgba(0, 0, 0, 0.2);
  }
  .gbs-menu-surface {
    position: absolute;
    top: 100%;
  ...
  .gbs-menu-surface .scrollable-content:after {
    position: absolute;
    bottom: 0;
  ...
  .gbs-menu-surface .scrollable-content :global(.gbs-menu-item) {
    width: 100%;
  }

  .gbs-menu-surface :global(.gbs-tooltip) {
